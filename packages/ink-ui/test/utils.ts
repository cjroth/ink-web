...
export function renderForTest(node: ReactNode, options?: Partial<RenderOptions>): TestRenderResult {
  ...
  const waitForRender = (): Promise<void> => {
    return new Promise((resolve) => {
      // If onRender has already been called, resolve immediately
      if (renderCount > 0) {
        resolve()
        return
      }
      // Otherwise wait for onRender to fire
...
export function setupHappyDom(): void {
  ...
  // Suppress spurious "duplicate key" warnings from ink's react-reconciler
  // These occur with useInput hook due to internal ink implementation details
