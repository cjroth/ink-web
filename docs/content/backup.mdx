...
```bash
npm install ink-web react ghostty-web
```
...
```bash
npm install ink-web ink react ghostty-web
```
...
const { term, unmount } = mountInk(<App />, { 
  container,
  focus: true 
})
...
const MyTerminal = () => (
  <InkTerminal className="terminal-container" focus>
    <App />
  </InkTerminal>
)
```
...
### `mountInk(element, options)`

Mounts an Ink component into a [ghostty-web](https://github.com/coder/ghostty-web) terminal.
...
React component wrapper for mounting Ink in a terminal.
...
The shims are minimal implementations that provide just enough functionality for Ink to render correctly in [ghostty-web](https://github.com/coder/ghostty-web). When you configure Vite to alias Node.js modules to these shims, Ink can run without modification.
