...
description: API reference for InkTerminalBox and InkTerminal components
---

ink-web provides two main components for rendering Ink applications in the browser: `InkTerminalBox` and `InkTerminal`. These components handle the ghostty-web integration and provide a terminal-like environment for your Ink components.

## InkTerminalBox

A higher-level component that wraps `InkTerminal` with styling, CSS isolation, and loading state support. This is the recommended component for most use cases.
...
| `termOptions` | `ITerminalOptions` | `{}` | Terminal options |
...
- **Loading States**: Show a custom loading indicator while the terminal initializes
...
A lower-level component that directly integrates ghostty-web with Ink. Use this when you need more control or don't want the additional styling from `InkTerminalBox`.
...
`InkTerminal` uses a robust initialization strategy:
...
For advanced use cases, you can use the `mountInk` function directly to create your own terminal integration.

```tsx
import { mountInk, Box, Text } from "ink-web";

const container = document.getElementById("terminal");

const { term, unmount } = mountInk(
  <Box>
    <Text>Hello</Text>
  </Box>,
  {
...
|----------|------|-------------|
| `term` | `Terminal` | The ghostty-web Terminal instance |
| `unmount` | `() => Promise<void>` | Cleanup function |

## Terminal Options

Both components accept `termOptions` which are passed to ghostty-web. Common options include:
...
See the [ghostty-web documentation](https://github.com/coder/ghostty-web) for all available options. ghostty-web is API-compatible with xterm.js.
...
| Custom styling/no CSS reset | `InkTerminal` |
| Advanced/custom integration | `mountInk` |
