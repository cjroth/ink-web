...
<Accordions type="multiple">
  ...
  <Accordion title="Garbled or flickering output with ink 6.6+">
    **Problem**: After upgrading your project's `ink` dependency to 6.6 or later, the terminal output in the browser is garbled, duplicated, or flickers on every render.

    **Cause**: Ink 6.6+ introduced two changes that are incompatible with xterm.js 5.x:

    1. **Synchronized update sequences (BSU/ESU)**: Ink 6.8.0 wraps every render in `\x1b[?2026h` (begin) and `\x1b[?2026l` (end) DEC private mode 2026 sequences when `stdout.isTTY === true`. xterm.js 5.x has no support for these sequences, so they are rendered as visible garbage or cause cursor positioning errors.

    2. **Rewritten incremental renderer**: Ink 6.8.0's incremental renderer uses new cursor math (`visibleLineCount`, `cursorUp(previousVisible - 1)`) that drifts when running inside xterm.js, causing duplicated or overlapping lines.

    **Solution**: Upgrade to `ink-web@0.1.17` or later, which is tested against ink 6.8.0 and strips the incompatible sequences.
