...
var wrapAnsi2 = (code, str, resetCode) => {
  return `\x1B[${code}m${str}\x1B[${resetCode}m`;
};
var createChalkInstance = () => {
  const chalk2 = (str) => str;
  Object.entries(ANSI_CODES).forEach(([name, [open, close]]) => {
    chalk2[name] = (str) => wrapAnsi2(open, str, close);
    const builder = (str) => wrapAnsi2(open, str, close);
    Object.entries(ANSI_CODES).forEach(([innerName, [innerOpen, innerClose]]) => {
      builder[innerName] = (str) => wrapAnsi2(open, wrapAnsi2(innerOpen, str, innerClose), close);
    });
    chalk2[name] = Object.assign(builder, builder);
  });
  chalk2.level = 3;
  chalk2.supportsColor = { level: 3, hasBasic: true, has256: true, has16m: true };
  return chalk2;
};
var chalk = createChalkInstance();
...
var colorize = (str, color, type) => {
  ...
  if (isNamedColor(color)) {
    if (type === "foreground") {
      return chalk_default[color](str);
    }
    const methodName = `bg${color[0].toUpperCase() + color.slice(1)}`;
    return chalk_default[methodName](str);
  }
  if (color.startsWith("#")) {
    return type === "foreground" ? chalk_default.hex(color)(str) : chalk_default.bgHex(color)(str);
  }
  if (color.startsWith("ansi256")) {
    ...
    return type === "foreground" ? chalk_default.ansi256(value)(str) : chalk_default.bgAnsi256(value)(str);
  }
  if (color.startsWith("rgb")) {
    ...
    return type === "foreground" ? chalk_default.rgb(firstValue, secondValue, thirdValue)(str) : chalk_default.bgRgb(firstValue, secondValue, thirdValue)(str);
  }
  return str;
};
...
var renderBorder = (x, y, node, output) => {
  if (node.style.borderStyle) {
    ...
    if (showTopBorder && dimTopBorderColor) {
      topBorder = chalk_default.dim(topBorder);
    }
    ...
    if (dimLeftBorderColor) {
      leftBorder = chalk_default.dim(leftBorder);
    }
    let rightBorder = (colorize_default(box.right, rightBorderColor, "foreground") + "\n").repeat(verticalBorderHeight);
    if (dimRightBorderColor) {
      rightBorder = chalk_default.dim(rightBorder);
    }
    ...
    if (showBottomBorder && dimBottomBorderColor) {
      bottomBorder = chalk_default.dim(bottomBorder);
    }
...
function Text({ color, backgroundColor, dimColor = false, bold = false, italic = false, underline = false, strikethrough = false, inverse = false, wrap = "wrap", children, "aria-label": ariaLabel, "aria-hidden": ariaHidden = false }) {
  ...
  const transform = (children2) => {
    if (dimColor) {
      children2 = chalk_default.dim(children2);
    }
    if (color) {
    ...
    if (bold) {
      children2 = chalk_default.bold(children2);
    }
    if (italic) {
      children2 = chalk_default.italic(children2);
    }
    if (underline) {
      children2 = chalk_default.underline(children2);
    }
    if (strikethrough) {
      children2 = chalk_default.strikethrough(children2);
    }
    if (inverse) {
      children2 = chalk_default.inverse(children2);
    }
    return children2;
  };
  if (isScreenReaderEnabled && ariaHidden) {
...
function mountInkInXterm(element, opts) {
  ...
  updateStreamsSize();
  const instance = render_default(element, { stdout, stderr: stdout, stdin, patchConsole: false, debug: false });
  return {
    term,
    unmount: async () => {
      try {
